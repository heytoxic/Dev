...
app.use(cors({
  origin: ['https://dev-1-bjb7.onrender.com', 'https://dev-yndg.onrender.com', 'http://localhost:3000', /\.onrender\.com$/],
  credentials: true
}));
...
// Always assign colors in standard order — all 4 corners always visible on board
// 2P: red vs blue (opposite corners), 3P: red+blue+green, 4P: all
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', raw => {
    ...
    switch(msg.type){
      ...
      // ── Google Auth — verify credential token ──
      case 'google_auth': {
        // Client sends either googleId+name+picture (already decoded on client)
        // or idToken for server-side verification
        const userId = 'g_' + (msg.googleId || uuidv4());
...
// ── Keep Render free tier alive (self-ping every 14 min) ──
// This prevents the 30s cold start which causes high ping spikes
const SELF_URL = process.env.RENDER_EXTERNAL_URL || `http://localhost:${PORT}`;
if(process.env.RENDER_EXTERNAL_URL) {
  setInterval(()=>{
    const http_or_https = SELF_URL.startsWith('https') ? require('https') : require('http');
    http_or_https.get(SELF_URL + '/health', (r)=>{ /* keepalive */ }).on('error',()=>{});
  }, 14 * 60 * 1000);
}
app.get('/health', (req,res)=>res.json({ok:true,rooms:rooms.size,clients:clients.size,ts:Date.now()}));
